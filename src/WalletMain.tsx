...
import { keccak256, HDNodeWallet, Wallet as weth } from "ethers"


export function WalletMain({ type = "sol", phrase, clear }) {
                        ...
                        <AccordionContent onClick={() => handleCopy()} className="md:px-10 px-5">
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                                {phrase.map((w: String, i: Number) => {
                                    return (<Button className="w-full" key={i} variant={"secondary"}>{w}</Button>)
                                })}
                            </div>
